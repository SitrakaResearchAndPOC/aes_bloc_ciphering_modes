            ...
            def crypte(self):
                if self.barre1.get() == "Fichier à crypter ou à décrypter...":
                    ...
                    if not self.hashage.get():
                        ...
                        if(self.mode.get() == 1):
                            #password = p_word.get()  # Get this from somewhere else like input()
                            self.key =  self.hashmotdepasse(self.password)
                            print(self.key)
                            #output_filename = self.fichier + ".GCM" +'.crypte'  # You can name this anything, I'm just putting .encrypted on the end
                            output_filename = self.fichier + ".crypte" # You can name this anything, I'm just putting .encrypted on the end
                            
                            # Open files
                            file_in = open(self.fichier, 'rb')  # rb = read bytes. Required to read non-text files
                            file_out = open(output_filename, 'wb')  # wb = write bytes. Required to write the encrypted data
                            
                            cipher = AES.new(self.key, AES.MODE_GCM)  # Create a cipher object to encrypt data
                            ...
                            #     encryption = Image.frombytes("RGB", (img.width, img.height),image_bytes)
                            #     encryption.show()
                            
                            # Close both files
                            file_in.close()
                            file_out.close()
                            messagebox.showinfo("Information","Votre fichier est crypté.")
                          
                        elif (self.mode.get() == 2):
                            self.key =  self.hashmotdepasse(self.password)
                            print(self.key)
                            output_filename = self.fichier + '.CFB' + '.crypte'  # You can name this anything, I'm just putting .encrypted on the end
                            with open (self.fichier,'rb') as entry:
                                data = entry.read()
                            ...
                            entry.close()
                            with open(output_filename,'w') as data:
                                data.write(to_write)
                            data.close()
                            messagebox.showinfo("Information","Votre fichier est crypté.")          
                            ...
                            self.key =  self.hashmotdepasse(self.password)
                            print(self.key)
                            output_filename = self.fichier +'.OFB' +'.crypte'  # You can name this anything, I'm just putting .encrypted on the end
                            ...
                            self.key =  self.hashmotdepasse(self.password)
                            print(self.key)
                            output_filename = self.fichier + '.CBC' +'.crypte'  # You can name this anything, I'm just putting .encrypted on the end
                            with open (self.fichier,'rb') as entry:
                                data = entry.read()
                            ...
                            self.key =  self.hashmotdepasse(self.password)
                            print(self.key)
                            output_filename = self.fichier + '.ECB' +'.crypte'  # You can name this anything, I'm just putting .encrypted on the end
                            with open (self.fichier,'rb') as entry:
                                data = entry.read()
                            ...
                            self.key =  self.hashmotdepasse(self.password)
                            print(self.key)
                            output_filename = self.fichier + '.CTR' + '.crypte'  # You can name this anything, I'm just putting .encrypted on the end
                            with open (self.fichier,'rb') as entry:
                                data = entry.read()
                ...
                if self.barre1.get() == "Fichier à crypter ou à décrypter...":
                    ...
                    if not self.hashage.get():
                        ...
                        if(self.mode.get() == 1):
                            #password = p_word.get()  # Get this from somewhere else like input()
                            #key=getkey(password)
                            #key=getkey1(password)
                            self.key =  self.hashmotdepasse(self.password)
                            print(self.key) 
                    
                            #input_filename = 'texte.txt.encrypted'  # The encrypted file
                            input_filename = self.fichier
                            output_filename = self.fichier + ".decrypte"    # You can name this anything, I'm just putting .encrypted on the end
                             # Open files
                            try:
                                file_in = open(self.fichier, 'rb')
                                ...
                                encrypted_data_size = file_in_size - 16 -16  # Total - salt - nonce - tag = encrypted data
                                ...
                                messagebox.showinfo("Information","Votre fichier est decrypté.")
                            ...
                            try:
                                ...
                                decrypted = cipher.decrypt(ciphertext)
                                decrypted = unpad(decrypted,AES.block_size)
                                with open ('decrypt.txt', 'wb') as data:
                                    data.write(decrypted)
                                    data.close()
                            except(ValueError,KeyError):
                                messagebox.showinfo("Information","Mot de passe incorrect.")
...
8.Clicker le boutton partage pour partager des fichiers""",)
